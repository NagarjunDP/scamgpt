...
export default function ReportScam() {
  ...
  // Clear transient error popup after 4s
  useEffect(() => {
    if (!errorMsg) return;
    ...
    try {
      ...
      try {
        ...
        if (dashRes.ok) {
          ...
          // Expecting { recent_threats: Threat[] }
          if (data?.recent_threats && Array.isArray(data.recent_threats)) {
            setRecentThreats(data.recent_threats.slice(0, 3));
          }
        }
      } catch (err) {
        console.error('Could not fetch recent threats', err);
      }
            ...
            <div className="space-y-3">
              {recentThreats.map((threat, idx) => (
                <motion.div
                  ...
                  onClick={() =>
                    setExpandedThreatId(expandedThreatId === threat.id ? null : threat.id)
                  }
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {threat.target && (
                        <span
                          className="text-zinc-500 text-xs truncate max-w-[150px] md:max-w-[200px]"
                          title={threat.target}
                        >
                    ...
                    {expandedThreatId === threat.id ? (
                      ...
                      >
                        <div className="pt-3 border-t border-zinc-800/60">
                          <span className="text-[10px] text-[#ff0033] uppercase font-bold tracking-wider mb-1 block">
                            Cognitive Summary / Report Details
                          </span>
                  ...
                  <div
                    className={`w-full border-2 border-dashed ${
                      file
                        ...
                        : 'border-zinc-700/50 bg-black/30 group-hover/upload:border-[#ff0033]/40 group-hover/upload:bg-[#ff0033]/5'
                    } rounded-xl h-24 flex flex-col items-center justify-center transition-all duration-300`}
                  >
                    <Upload
                      className={`w-6 h-6 mb-2 ${
                        file ? 'text-[#ff0033]' : 'text-zinc-500 group-hover/upload:text-[#ff0033]/70'
                      }`}
                    />
